...
# CORE SYNC RELAY
# ═══════════════════════════════════════════════════════════
# Empfängt Live-Regel-Injections von G-CORE (Cloud).
# Empfängt Vektor-Syncs von G-CORE → lokale ChromaDB.
# Schreibt direkt in .cursor/rules/CORE_LIVE_INJECT.mdc
# ═══════════════════════════════════════════════════════════
...
TARGET_FILE = ".cursor/rules/CORE_LIVE_INJECT.mdc"
...
if not SECRET:
    logging.warning("CORE_WEBHOOK_SECRET not found in .env! Relay is insecure/broken.")

async def handle_inject(request):
    """
    Endpoint: POST /inject
    Header: X-Core-Secret: <CORE_WEBHOOK_SECRET>
    Body: {"content": "..."}
    """
    # 1. Security Check
    auth_header = request.headers.get('X-Core-Secret')
            ...
            try:
                ...
                if add_proc.returncode != 0:
                    ...
                    commit_proc = await asyncio.create_subprocess_exec(
                        "git", "commit", "-m", "chore(sync): CORE_LIVE_INJECT.mdc via /inject",
                        cwd=ROOT,
                        stdout=asyncio.subprocess.DEVNULL,
                        stderr=asyncio.subprocess.PIPE,
                    )
...
if __name__ == '__main__':
    logging.info(f"Starting CORE SYNC RELAY on port 8049...")
