...
"""
mTLS-Zertifikats-Generator für CORE GQA Refactor F3 (unified-auth-mtls).

Erzeugt:
- Root CA (core-ca)
- Server CA (core-srv) und Client CA (core-cli)
- Server-Zertifikate: core-api, mcp-server, openclaw-server
- Client-Zertifikate: cursor, scout, oc-brain, ha, core-client
...
def _subject(cn: str, o: str = "CORE") -> x509.Name:
    return x509.Name([
...
def create_root_ca(out_dir: Path, days: int = 3650) -> tuple[x509.Certificate, rsa.RSAPrivateKey]:
    """Root CA (10 Jahre)."""
    key = _make_key()
    subject = issuer = _subject("core-ca.local")
    cert = (
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CORE mTLS Zertifikats-Generator")
    ...
    # 1. Root CA
    print("  [1/6] Root CA (core-ca)...")
    root_cert, root_key = create_root_ca(out_dir)

    # 2. Intermediate CAs
    print("  [2/6] Server CA (core-srv)...")
    srv_ca_cert, srv_ca_key = create_intermediate_ca(
        "srv", "core-srv.local", root_cert, root_key, out_dir
    )
    print("  [3/6] Client CA (core-cli)...")
    cli_ca_cert, cli_ca_key = create_intermediate_ca(
        "cli", "core-cli.local", root_cert, root_key, out_dir
    )
    ...
    create_server_cert(
        "core-api",
        "core-api.dreadnought.local",
        ["core-api.local", "localhost", "127.0.0.1"],
        srv_ca_cert, srv_ca_key, out_dir, days,
    )
    create_server_cert(
        "mcp-server",
        "mcp.vps.core.local",
        ["mcp.vps.core.local", "localhost", "127.0.0.1"],
        srv_ca_cert, srv_ca_key, out_dir, days,
    )
    create_server_cert(
        "openclaw-server",
        "openclaw.vps.core.local",
        ["openclaw.vps.core.local", "localhost", "127.0.0.1"],
        srv_ca_cert, srv_ca_key, out_dir, days,
    )
    ...
    for name, cn in [
        ...
        ("oc-brain", "oc-brain.vps.core.local"),
        ("ha", "ha.scout.local"),
    ...
    print("  Server: core-api.pem/.key, mcp-server.pem/.key, openclaw-server.pem/.key")
    print("  Client: cursor.pem/.key, scout.pem/.key, oc-brain.pem/.key, ha.pem/.key, core-client.pem/.key")
