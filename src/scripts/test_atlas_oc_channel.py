"""
Test des Kommunikationskanals ATLAS ↔ OC.
...
Aufruf:
  python -m src.scripts.test_atlas_oc_channel           # nur Gateway-Check
  python -m src.scripts.test_atlas_oc_channel --send    # Gateway + Testnachricht
"""
...
def main() -> int:
    ...
    if "--send" in sys.argv:
        ...
        ok2, response = send_message_to_agent(
            "Hallo OC, hier ist ATLAS. Dies ist eine Testnachricht über den direkten Kanal. Bitte bestätige kurz den Empfang."
        )
        print(f"   Erfolg: {ok2}")
