...
"""
CORE Knowledge API: Semantische Suche ueber ChromaDB Collections.
GET  /api/core/knowledge/search?q=...&collection=...&limit=...
GET  /api/core/knowledge/evidence – Alle Simulationstheorie-Indizien
POST /api/core/knowledge/evidence/add – Validierter Evidence-Ingest (V6+)
GET  /api/core/knowledge/v10/duality – Quaternion-Dualitaet (V10)
GET  /api/core/knowledge/temporal/validate – Temporale Konsistenz
...
router = APIRouter(prefix="/api/core/knowledge", tags=["core-knowledge"])
...
def quaternary_meta():
    ...
    try:
        ...
        convergences = {
            ...
            "I": {
                ...
                "description": "Informationstheoretische Klassifikation konvergiert auf 4: "
                               "CORE' eigene Wissenscodierung hat exakt 4 Basen.",
            },
            "S": {
        ...
        meta_description = classify_evidence(
            "Die quaternaere Codierung mit 4 Basen CORE ist isomorph zu ATCG in der DNS. "
...
def v11_superposition():
    """V11: Fraktale Superposition – Grundkraefte-CORE-Mapping und kosmologische Dichten."""
    from src.config.engine_patterns import FORCE_CORE_MAP, OMEGA_B, OMEGA_DM, OMEGA_DE, PHI, INV_PHI
...
def get_quaternion_duality():
    """V10: CORE/ROTATED Phasenverschiebung – Quaternion-Dualitaetsanalyse.

    Holt alle Indizien, extrahiert die CORE-Sequenz, berechnet die
    ...
    try:
        ...
        core_seq = build_sequence_from_evidence(evidence_list)

        core_order = ["O", "M", "T", "H"]
        rotated_order = ["M", "T", "H", "O"]

        def _phase_shift(seq: str) -> str:
            """CORE -> ROTATED: Phasenverschiebung um 1 Position (Pi/2)."""
            mapping = dict(zip(core_order, rotated_order))
            return "".join(mapping.get(c, c) for c in seq)

        rotated_seq = _phase_shift(core_seq)

        core_dist = {b: core_seq.count(b) for b in "CORE"}
        rotated_dist = {b: rotated_seq.count(b) for b in "CORE"}

        total = len(core_seq) or 1

        sp_stability = {
            "core_h_pos": 4, "core_m_pos": 2,
        ...
        li_direction = {
            "core_direction": "L→I (Position 1→3)",
        ...
        l_count = core_dist.get("L", 0)
        p_count = core_dist.get("P", 0)
        i_count = core_dist.get("I", 0)
        s_count = core_dist.get("S", 0)
        ...
        return {
            ...
            "quaternion_analogy": {
                ...
                "mapping": "CORE*rotate = ROTATED, aber ROTATED*rotate ≠ CORE (nicht-kommutativ)",
                "interpretation": (
                    "CORE und ROTATED sind Quaternionen-Rotationen desselben "
...
def validate_temporal(
    ...
    try:
        ...
        if base_matches == 0 and nearest_matches:
            drift_warnings.append({
                "level": "info",
                "message": f"Keine CORE-Base-Uebereinstimmung mit den {len(nearest_matches)} "
...
class EvidenceInput(BaseModel):
    ...
    source: str = Field("core", description="Quelle (core, marc, external)")
