...
async def receive_ha_action(
    ...
    # Ring-0: Telemetry-Injector Input-Triage (Validation Sync - Wrapped in Thread)
    # Telemetry-Injector acts as the initial Logic Validator on the Strut
    telemetry_injector_result = None
    try:
        def _run_hugin():
            from src.logic_core.telemetry_injector import triage
            return triage(entry)

        telemetry_injector_result = await asyncio.to_thread(_run_hugin)
        logger.info(f"Telemetry-Injector Triage: core={telemetry_injector_result.core_base} intent={telemetry_injector_result.intent} priority={telemetry_injector_result.priority}")
    except Exception as e:
        logger.warning(f"Telemetry-Injector Triage fehlgeschlagen: {e}")
    ...
    if action == "core_ping":
        reply_text = "CORE: Ping empfangen! Brücke zur App funktioniert."
        ha_client.send_mobile_app_notification(reply_text, title="PONG")
        return {"status": "ok", "action": "ping_replied"}

    elif action == "core_command" or action == "text_input":
        ...
        if SCOUT_DIRECT_MODE:
            ...
            if telemetry_injector_result is not None:
                ctx["telemetry_injector_triage"] = telemetry_injector_result
        ...
        ha_client.send_mobile_app_notification(reply_text, title="CORE")
...
def _legacy_ha_command_pipeline(user_text: str) -> str:
    ...
    triage = core_llm.run_triage(user_text)
    ...
    if triage.intent in ["deep_reasoning", "chat"]:
        sys_prompt = "Du bist Virtual Marc, Kopf des Core Councils für CORE. Antworte analytisch, auf Systemik fokussiert. Meide neurotypische Floskeln vollständig."
        ...
        try:
            ...
            return core_llm.invoke_heavy_reasoning(sys_prompt, user_text)
...
def _forwarded_text_pipeline(text: str) -> str:
    ...
    if triage.intent in ["deep_reasoning", "chat"]:
        sys_prompt = "Du bist Virtual Marc, Kopf des Core Councils für CORE. Antworte analytisch, auf Systemik fokussiert."
        try:
            from src.logic_core.context_injector import inject_context_for_agent, check_semantic_drift, apply_veto
...
async def assist_pipeline(
    ...
    if reply:
        try:
            ...
            await dispatch_tts(text=reply, target=tts_target, role_name="core_dialog")
        except Exception as e:
