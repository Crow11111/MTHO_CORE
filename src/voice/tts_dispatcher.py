...
- piper: Piper TTS lokal (Fallback wenn ElevenLabs nicht verfügbar)

Fallback-Kette: elevenlabs* → Piper (lokal) → mini (HA TTS)
"""
...
# ATLAS-Default-Stimme (atlas_dialog aus voice_config)
DEFAULT_ATLAS_VOICE_ID = "0ISBUrWf7OGBgepl5lu2"
...
async def _elevenlabs_stream_to_mini(
    ...
    Der Mini streamt die MP3 von ATLAS_HOST_IP:PORT.
    """
    ...
    host_ip = os.getenv("ATLAS_HOST_IP", "192.168.178.110")
...
async def dispatch_tts(
    ...
    if target == "piper":
        ...
        logger.warning("Piper nicht verfügbar, Fallback auf mini.")
        return await _mini_tts(text)
