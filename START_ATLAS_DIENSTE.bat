...
call :port_listening 8088
if errorlevel 1 (
  echo [1/3] Backend Port 8088... gestartet.
  start "ATLAS Backend" cmd /k "python -m uvicorn src.api.main:app --host 0.0.0.0 --port 8088"
) else (
  echo [1/3] Backend Port 8088... Port belegt, ueberspringe.
)

call :port_listening 8501
if errorlevel 1 (
  echo [2/3] Dev-Agent Port 8501... gestartet.
  start "ATLAS Dev-Agent" cmd /k "python -m streamlit run src/ui/dashboard.py --server.port 8501"
) else (
...
if errorlevel 1 (
  ...
  start "ATLAS Voice-Info" cmd /k "python -m streamlit run src/ui/voice_info_console.py --server.port 8502"
) else (
