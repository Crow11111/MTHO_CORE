# WhatsApp: Trigger & Adressierung – Plan (ATLAS ↔ OC ↔ Dev-Agent)
...
Abstimmung zwischen **ATLAS** (Webhook/HA-Pfad), **OC** (OpenClaw) und **Dev-Agent** (Tests/Doku). Dieser Plan ist die gemeinsame Basis; OC und Dev-Agent können unten ergänzen.
...
|--------|----------------|----------------------------|
| **HA-Pfad (ATLAS)** | **Alle** eingehenden Nachrichten in Marcs Account (Addon meldet jedes Event). Also: Schwester → Marc, Kollege → Marc, Marc → sich selbst, etc. | **Hoch:** Ohne Filter würde ATLAS auf jede Nachricht antworten. |
...
**Kernproblem HA-Pfad:** Das Addon liefert **jede** eingehende Nachricht an ATLAS. Es gibt keine „zweite Nummer“ – alles läuft über Marcs Account. Daher: **Trigger nötig**, damit nur gewollte Nachrichten eine System-Antwort auslösen (aktuell: @Atlas / @OC).

---

## 2. Anforderungen (kurz)

- **Adressierung:** @Atlas → ATLAS/Scout; @OC → OC. Nur der Adressierte antwortet (plus Bedenken-Pflicht). Siehe [WHATSAPP_ROUTING_ATLAS_OC.md](WHATSAPP_ROUTING_ATLAS_OC.md).
- **Alles lesen:** Beide Systeme dürfen Nachrichten lesen können (für Kontext/Bedenken); Lesen ≠ Antworten.
- **Keine systemische Antwort ohne Trigger:** Wenn eine Nachricht **für Marc persönlich** bestimmt ist (z. B. von der Schwester, ohne @Atlas/@OC), darf **kein** System (ATLAS, OC) automatisch antworten. Trigger = explizite Ansprache (z. B. @Atlas, @OC) oder später: eigener Dienst / Allowlist.
...
- Nachricht **beginnt mit @Atlas** → ATLAS darf antworten.
- Nachricht **beginnt mit @OC** → nur OC (ATLAS ignoriert).
...
### B) Allowlist Absender (HA / ATLAS)

- **Vor** dem Aufruf des ATLAS-Webhooks: Nur wenn `remoteJid` / Absender in einer erlaubten Liste steht (z. B. nur Marcs Nummer beim Schreiben von anderem Gerät), wird der Webhook aufgerufen. Alle anderen Nachrichten werden nicht an ATLAS weitergeleitet.
...
- **Zwei Nummern:** Eine Nummer nur für „System“ (ATLAS/OC), eine für privaten Chat. Nur Nachrichten an die System-Nummer werden verarbeitet. So sieht das System nie Chats, die nur für Marc bestimmt sind.
...
- Trigger- und Routing-Logik in einen **eigenen Dienst** auslagern (zwischen HA-Event und ATLAS/OC). Dieser Dienst entscheidet: Weiterleiten oder nicht, an wen, mit welchem Kontext.
...
- **ATLAS (HA-Pfad):** Sieht alle Nachrichten, die ins Addon kommen. Reagiert nur bei @Atlas am Anfang (bzw. bei @OC am Anfang: nicht, außer Bedenken). Implementiert in `whatsapp_webhook.py`.
...
1. **Trigger-Regel beibehalten und dokumentieren**  
   @Atlas / @OC am Anfang; keine Antwort ohne Trigger. Doku: [WHATSAPP_ROUTING_ATLAS_OC.md](WHATSAPP_ROUTING_ATLAS_OC.md). Bereits umgesetzt.
...
4. **Eigener Dienst (später)**  
   Trigger- und Routing-Logik in eigenen Dienst auslagern; siehe [WHATSAPP_ROUTING_ATLAS_OC.md](WHATSAPP_ROUTING_ATLAS_OC.md) – Offene Punkte.

5. **Tests / Checkliste (Dev-Agent)**  
   - Nachricht ohne Trigger (z. B. „Hallo“ von Test-Absender) → keine ATLAS-/OC-Antwort.  
   - Nachricht mit @Atlas → ATLAS antwortet mit [ATLAS]/[Scout].  
   - Nachricht mit @OC → ATLAS antwortet nicht (OC antwortet, wenn konfiguriert).  
...
| Wen | Wie | Skript |
|-----|-----|--------|
| **OC** | Eine Nachricht mit der Aufgabe wird **per API** an das OpenClaw-Gateway geschickt (ATLAS → OC). OC erhält sie im gleichen Kanal wie andere Eingaben und kann antworten oder eine Einreichung in `rat_submissions/` ablegen. | `python -m src.scripts.trigger_whatsapp_plan_oc` |
...
- **OC:** Kein SSH nötig – die Kommunikation läuft über die bestehende Gateway-API (`send_message_to_agent`). Das Skript sendet die Aufgabentexte (inkl. relevanter Plan-Auszüge) an den Agenten „main“. OC soll seine Antwort als **rat_submission** ablegen (topic z. B. „WhatsApp-Plan Abschnitt 6“); ATLAS holt sie mit `fetch_oc_submissions` ab.
...
- **Wer führt aus:** Entweder **OC** (wenn er Zugriff hat) oder **ATLAS** (z. B. Skript `enable_oc_responses_vps` per SSH vom Rechner aus). Ziel: dass es klappt.
...
Damit die Kommunikation mit OC per **Logikketten/Logikschalter** auslösbar ist (ohne Skript von Hand), bietet ATLAS ein Endpoint, das API-Versuch und Fallback bündelt:
...
**Frage:** Würde es viele Kommunikationsprobleme erleichtern, wenn auf dem Hostinger-Server **auch** eine HA-Instanz läuft? Dann wären HA, OC (OpenClaw) und ggf. ein schlanker ATLAS-Proxy auf **einem** Host – alle auf dem **gleichen Bus** (gleiches Netz, localhost oder lokales LAN).
...
- **Vorteile:** Eine zentrale HA-Instanz auf Hostinger könnte z. B. (1) OC und ggf. ATLAS-Dienste per localhost/Inter-Container ansprechen, (2) WhatsApp-Addon oder -Integration an einem Ort bündeln, (3) Logikschalter und Abstimmungs-Automations ohne Umweg über den Scout/Dreadnought betreiben.
...
- **Konkrete Tests:** (1) Webhook & Trigger: `tests/test_whatsapp_webhook.py` mit TestClient – Cases: „Hallo“ → ignoriert, „@Atlas Ping“ → Verarbeitung, „@OC Ping“ → ATLAS antwortet nicht. (2) E2E HA: `src/scripts/test_whatsapp_e2e_ha.py`. (3) OpenClaw E2E: `src/scripts/test_openclaw_e2e.py` mit openclaw_client. (4) Optional Allowlist: `tests/test_whatsapp_allowlist.py` bei Umsetzung.
...
- [WHATSAPP_ROUTING_ATLAS_OC.md](WHATSAPP_ROUTING_ATLAS_OC.md) – Routing, Bedenken-Pflicht, Offene Punkte
- [WHATSAPP_E2E_HA_SETUP.md](WHATSAPP_E2E_HA_SETUP.md) – HA-Setup, Automation, 2.1 Routing
- [WHATSAPP_OPENCLAW_VS_HA.md](WHATSAPP_OPENCLAW_VS_HA.md) – OC vs. HA-Pfad, allowFrom
- [DEV_AGENT_UND_SCHNITTSTELLEN.md](DEV_AGENT_UND_SCHNITTSTELLEN.md) – Netzarchitektur, Addon, OC
- `src/api/routes/whatsapp_webhook.py` – ATLAS-Trigger-Logik (@Atlas / @OC am Anfang)
