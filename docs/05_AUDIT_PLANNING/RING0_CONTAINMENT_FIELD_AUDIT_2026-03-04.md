...
|------------|--------|-----------------|
| Hugin (Logik & Scout) | FAIL | Nicht als dedizierte Ring-0-Komponente implementiert |
| Munin (Kontext & Validierung) | FAIL | Nicht implementiert |
| Munin Veto (Semantic Drift Block) | FAIL | Nicht implementiert |
...
**Erwartung:** Kontext-Validierung, Wuji-Archiv, semantische Konsistenz.

**Befund:** `[FAIL]` Munin ist **nicht implementiert**. Kein Wuji-Archivor, keine Kontext-Validierung vor Schreibzugriffen auf Ring-0-Collections (`core_directives`, `simulation_evidence`).
...
- Die Route `POST /api/atlas/knowledge/evidence/add` (in `atlas_knowledge.py`) schreibt in `simulation_evidence` – sie ist **nicht** in `main.py` registriert (aktuell tot), aber falls sie aktiviert würde: **kein Council-Gate-Match** (Pfad nicht in `CRITICAL_PATH_PATTERNS`), **keine Auth** für Ring-0-Writes.
...
| API (falls aktiviert) | `POST /api/atlas/knowledge/evidence/add` nicht in Council-Gate-Patterns |
...
**Nicht abgedeckt:** Ring-0-Writes (`/api/atlas/knowledge/evidence/add`, `/api/atlas/knowledge/directive/add` o.ä.).
...
2. **Hugin/Munin nicht implementiert:** Konzeptionell in der Theorie, fehlen im Code.
...
3. **CRITICAL_PATH_PATTERNS erweitern:** `/api/atlas/knowledge/evidence/add`, `/api/atlas/knowledge/directive/add` (falls diese Routen aktiviert werden) in die Council-Gate-Patterns aufnehmen.

### Priorität 2 (Hoch)

4. **Munin Veto / Semantic Drift:** Vor Ring-0-Writes `temporal_validator.validate_temporal_consistency` aufrufen; bei Konsistenz-Score unter Schwellwert (z.B. 0.3) → Block mit `HARD_REJECT_PROMPT`.
...
6. **Hugin/Munin als Module:** Klare Trennung: Hugin = Validierungs-Layer vor Logik-Execution; Munin = Kontext-Check vor Archiv-Writes. Dokumentation in Code und Architektur-Docs.
