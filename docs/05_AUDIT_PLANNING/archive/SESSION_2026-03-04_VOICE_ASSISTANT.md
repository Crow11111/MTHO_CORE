...
# Session 2026-03-04: CORE Voice Assistant Integration
...
Diese Session umfasste die vollständige Integration des CORE Voice Assistant Systems mit Home Assistant. Kernpunkte:

- **GQA (Gravitational Query Architecture) Refactor** – 8 Tasks für Zero-State-native Architektur
...
- **CORE API Server** – Port 8088 auf 4D_RESONATOR (CORE), Webhook-Authentifizierung
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                        CORE Voice Assistant System                              │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│   ┌─────────────────┐    ┌─────────────────────────────────────────────────────┐│
│   │     SCOUT       │    │                   DREADNOUGHT                       ││
│   │ (Raspi 5 / HA)  │    │                (CORE Host)                    ││
│   │ 192.168.178.54  │    │               192.168.178.110                       ││
│   ├─────────────────┤    ├─────────────────────────────────────────────────────┤│
│   │                 │    │                                                     ││
│   │ openWakeWord    │    │  CORE API (Port 8088)                              ││
│   │ "hey core"     │    │  ├── /webhook/inject_text (Conversation Agent)     ││
│   │       │         │    │  ├── /webhook/assist (TTS-triggering)              ││
│   │       ▼         │    │  └── /webhook/ha_action (Companion App)            ││
│   │ Whisper STT     │    │           │                                         ││
│   │       │         │    │           ▼                                         ││
│   │       ▼         │    │  ┌───────────────────────────────────────┐          ││
│   │ CORE Conver-   │────┼─►│ scout_direct_handler.process_text()  │          ││
│   │ sation Agent    │    │  │   ├── smart_command_parser (NL→HAAction)        ││
│   │       │         │    │  │   ├── Telemetry-Injector Triage (command/chat/deep)          ││
...
└─────────────────────────────────────────────────────────────────────────────────┘
    ...
    Datenfluss:
    ───────────
    1. User: "Hey CORE, Regal 80% Helligkeit"
    2. openWakeWord → Wake Word erkannt
    3. Whisper STT → "Regal 80% Helligkeit"
    4. CORE Conversation Agent → POST /webhook/inject_text
...
| `ha_integrations/atlas_conversation/api.py` | `AtlasApiClient` – HTTP-Client für CORE API |
...
### 3.2 CORE API Endpoints (Webhook-Routes)
...
| Ring-0 | Telemetry-Injector Input-Triage | ✅ |
| Ring-0 | Context-Injector Context Injection | ✅ |
| Ring-1 | Perf: asyncio.to_thread für Sync-Calls | ✅ |
| – | CORE Conversation HA Integration | ✅ |
...
| Host | IP | Rolle |
|------|----|-------|
| **4D_RESONATOR (CORE)** | `192.168.178.110` | CORE API Server |
...
| Service | Port | Host |
|---------|------|------|
| CORE API | `8088` | 4D_RESONATOR (CORE) |
| HA REST API | `8123` (HTTPS) | Scout |
| TTS Stream | `8002` | 4D_RESONATOR (CORE) (CORE_HOST_IP) |

### 4.3 Umgebungsvariablen (.env auf 4D_RESONATOR (CORE))
...
| `CORE_HOST_IP` | IP für Audio-Stream | `192.168.178.110` |
...
### 5.1 Installation der CORE Conversation Integration

1. **Dateien kopieren:**
   ```bash
   # Von 4D_RESONATOR (CORE) (Samba/SSH):
...
3. **Integration hinzufügen:**
   ...
   - "CORE Conversation" suchen
   - CORE API URL: `http://192.168.178.110:8088`
...
2. **Name:** "CORE"
3. **Conversation Agent:** CORE Conversation
4. **Speech-to-Text:** Whisper
5. **Text-to-Speech:** Piper
6. **Wake Word:** openWakeWord ("hey core")
...
4. Agent ID: CORE Conversation Entity

### 6.3 Voice-Pipeline E2E

1. Wake Word sagen: "Hey CORE"
...
- [ ] **Custom Wake-Word Model** – "hey core" oder "core" trainieren (openWakeWord)
...
- [ ] **HACS-Repository** – CORE Conversation als Custom Repo verfügbar machen
...
| 12:00 | CORE Conversation HA Integration erstellt |
| 14:00 | Samba/SSH-Zugriff auf Scout konfiguriert |
| 15:30 | Integration auf Scout deployed |
| 16:00 | CORE API Server auf Port 8088 gestartet |
