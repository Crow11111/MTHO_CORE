# CEO-Brief: G-CORE Git/Cursor Option 5 – Teamleads

**Erstellt:** 2026-03-06
**Auftraggeber:** CEO (Orchestrator)
**Mission:** Option 5 umsetzen – echter Push/Pull-Kreislauf zwischen G-CORE, Sync Relay, GitHub und Cloud Agents. Kein manueller Commit/Push mehr; VPS/Agents bei jedem Push automatisch aktuell.

**Referenz:** [G_CORE_GIT_CURSOR_OPTIMIERUNG.md](../02_ARCHITECTURE/G_CORE_GIT_CURSOR_OPTIMIERUNG.md) (Option 5 = Kombination 1+2, optional 4).
...
| **Phase 3:** Archive (Doku) | 400 | 1.000 | G_CORE_CIRCLE, Schnittstellen, Session-Log. |
...
- **Sync Relay** schreibt nach `/inject` wie bisher `CORE_LIVE_INJECT.mdc`, führt danach **optional** `git add` / `commit` / `push` aus → jede Injection landet sofort auf GitHub.
- **GitHub-Webhook** ruft bei Push einen Endpoint auf (VPS oder 4D_RESONATOR); der Endpoint führt **git pull** im CORE-Clone aus → Cloud Agents haben ohne Polling den neuesten Stand.
...
|------|--------|--------|-----------|
| **Architect** | Build-Engine / System | Sync Relay erweitern: nach erfolgreichem Schreiben von `CORE_LIVE_INJECT.mdc` optional Git-Befehle (add, commit, push). Konfiguration über Env; keine Credentials im Code. | Spezifikation/Änderung in `core_sync_relay.py`, Env-Variablen definiert, Fehlerbehandlung und Logging. |
...
| **API/Infra** | Build-Engine / Agency | Neuer Endpoint `POST /webhook/github`: GitHub-Payload entgegennehmen, HMAC (X-Hub-Signature-256) prüfen, bei `push`-Event `git pull` in konfigurierbarem Verzeichnis ausführen. Ort: Sync Relay (8049) oder FastAPI (main). | Implementierung Webhook-Handler, Doku für GitHub-Webhook-Eintrag (URL, Secret). |
| **Archive** | Retention | Nach Umsetzung: `G_CORE_CIRCLE.md` und ggf. `CORE_SCHNITTSTELLEN_UND_KANAALE.md` anpassen – Ablauf „F→Git“ als automatisch, „Git→CA“ als webhook-getriggert. Session-Log oder Audit-Plan aktualisieren. | Doku-Stand konsistent mit neuem Ablauf. |
...
3. **Archive:** Doku-Update (G_CORE_CIRCLE, Schnittstellen, Session-Log).
...
- Nach `/inject` liegt `CORE_LIVE_INJECT.mdc` automatisch im Repo (wenn Option aktiv).
