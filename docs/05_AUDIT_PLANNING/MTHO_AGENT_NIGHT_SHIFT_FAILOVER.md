...
Aktuell ist die MTHO Voice-Pipeline in Home Assistant (`ha_integrations/atlas_conversation`) hart an die IP von 4D_RESONATOR (MTHO_CORE) (`192.168.178.20:8000`) gebunden. 
Wenn 4D_RESONATOR (MTHO_CORE) (der lokale Entwicklungs-PC) ausgeschaltet wird, bricht die Voice-Pipeline zusammen. Home Assistant wirft einen Verbindungsfehler.

## 2. DAS ZIEL (TARGET STATE)
4D_RESONATOR (MTHO_CORE) ist der "Turbo" (Ring-1), nicht das Rückgrat. Das Rückgrat ist der **VPS (OpenClaw / OMEGA_ATTRACTOR)**.
Wir benötigen eine **Zero-Downtime Failover-Logik** direkt in der Home Assistant Custom Integration.

Wenn 4D_RESONATOR (MTHO_CORE) nicht antwortet (Timeout oder Connection Refused), muss Scout (HA) die Anfrage völlig transparent an das OMEGA_ATTRACTOR auf dem VPS routen.
...
2. Implementiert in `AtlasApiClient.async_send_text`:
   - Versuch 1: POST an `base_url` (4D_RESONATOR (MTHO_CORE)). Timeout auf sehr kurz (z.B. 2 Sekunden) setzen, um Latenzen zu vermeiden.
   - Wenn Timeout/Error: Catch Exception, logge "4D_RESONATOR (MTHO_CORE) offline, rerouting to Backbone (VPS)..."
