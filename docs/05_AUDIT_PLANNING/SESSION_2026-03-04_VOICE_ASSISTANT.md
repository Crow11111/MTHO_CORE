# Session 2026-03-04: ATLAS Voice Assistant Integration
...
Diese Session umfasste die vollständige Integration des ATLAS Voice Assistant Systems mit Home Assistant. Kernpunkte:
...
- **ATLAS API Server** – Port 8088 auf Dreadnought, Webhook-Authentifizierung
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                        ATLAS Voice Assistant System                              │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                  │
│   ┌─────────────────┐    ┌─────────────────────────────────────────────────────┐│
│   │     SCOUT       │    │                   DREADNOUGHT                       ││
│   │ (Raspi 5 / HA)  │    │                (ATLAS_CORE Host)                    ││
│   │ 192.168.178.54  │    │               192.168.178.110                       ││
│   ├─────────────────┤    ├─────────────────────────────────────────────────────┤│
│   │                 │    │                                                     ││
│   │ openWakeWord    │    │  ATLAS API (Port 8088)                              ││
...
└─────────────────────────────────────────────────────────────────────────────────┘
    ...
    Datenfluss:
    ───────────
    1. User: "Hey ATLAS, Regal 80% Helligkeit"
    2. openWakeWord → Wake Word erkannt
    3. Whisper STT → "Regal 80% Helligkeit"
    4. ATLAS Conversation Agent → POST /webhook/inject_text
...
| `ha_integrations/atlas_conversation/api.py` | `AtlasApiClient` – HTTP-Client für ATLAS API |
...
### 3.2 ATLAS API Endpoints (Webhook-Routes)
...
| – | ATLAS Conversation HA Integration | ✅ |
...
| Host | IP | Rolle |
|------|----|-------|
| **Dreadnought** | `192.168.178.110` | ATLAS_CORE API Server |
...
| Service | Port | Host |
|---------|------|------|
| ATLAS API | `8088` | Dreadnought |
| HA REST API | `8123` (HTTPS) | Scout |
| TTS Stream | `8002` | Dreadnought (ATLAS_HOST_IP) |

### 4.3 Umgebungsvariablen (.env auf Dreadnought)
...
### 5.1 Installation der ATLAS Conversation Integration

1. **Dateien kopieren:**
   ```bash
   # Von Dreadnought (Samba/SSH):
...
3. **Integration hinzufügen:**
   ...
   - "ATLAS Conversation" suchen
   - ATLAS API URL: `http://192.168.178.110:8088`
...
2. **Name:** "ATLAS"
3. **Conversation Agent:** ATLAS Conversation
...
4. Agent ID: ATLAS Conversation Entity

### 6.3 Voice-Pipeline E2E

1. Wake Word sagen: "Hey ATLAS"
...
- [ ] **HACS-Repository** – ATLAS Conversation als Custom Repo verfügbar machen
...
| 12:00 | ATLAS Conversation HA Integration erstellt |
| 14:00 | Samba/SSH-Zugriff auf Scout konfiguriert |
| 15:30 | Integration auf Scout deployed |
| 16:00 | ATLAS API Server auf Port 8088 gestartet |
...
*Erstellt: 2026-03-04*
*Autor: ATLAS Orchestrator (Session-Dokumentation)*
