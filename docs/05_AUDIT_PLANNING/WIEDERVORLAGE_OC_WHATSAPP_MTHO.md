...
# Wiedervorlage: WhatsApp Pairing + MTHO-Weiterbau

**Angelegt:** 28.02.2026  

---

## MTHO-Pipeline (ohne Warten auf WhatsApp)
...
## 2. MTHO funktional weiterbauen

Ziel = **funktionales Tool** wie in MTHO Neocortex V1.0 und Schnittstellen-Doc beschrieben:

- **Scout → OMEGA_ATTRACTOR:** Webhook POST `/v1/responses` mit Event-JSON (source, node_id, event_type, data).
- **OMEGA_ATTRACTOR:** Logik (4D_RESONATOR (MTHO_CORE)-Status), ChromaDB/State, Eskalation per WhatsApp (`[MTHO-ALERT]`).
- **Kanäle:** 4D_RESONATOR (MTHO_CORE)↔OC (bereits), rat_submissions (bereits), WhatsApp (nach Pairing), Scout-Webhook (Endpoint steht; Scout-Seite anbinden).
