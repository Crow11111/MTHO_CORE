...
**Generiert am:** 2026-03-06 11:11:06
...
- [00 CORE PROCESSES MASTER](#00-core-processes-master)
...
- [STAMMDOKUMENTE DEPLOY](#stammdokumente-deploy)
...
<a name="00-core-processes-master"></a>
# 00 CORE PROCESSES MASTER
...
| **core_knowledge/search (collection=all)** | 4 | ~300–900 ms | Sequentiell |
...
- core_knowledge/search: 4 Hops (3× ChromaDB sequentiell)

##### Synchrone Blockaden
| Komponente | Aufrufer | Problem |
|------------|----------|---------|
| `process_text()` | ha_webhook | Blockiert Event-Loop (Triage + HA/OC) |
| `core_llm.run_triage()` | ha_webhook, whatsapp_webhook | Sync Ollama-Call |
...
| ChromaDB query_* | core_knowledge | Sync, 3× sequentiell |
...
| core_knowledge/search (collection=all) | 300–900 ms | 3 ChromaDB-Queries parallel | ~60–70 % (900→300 ms) |
...
2. **core_knowledge/search**: ChromaDB-Queries parallel via `asyncio.gather` + `run_in_executor`
...
<a name="stammdokumente-deploy"></a>
## STAMMDOKUMENTE DEPLOY

### Stammdokumente für OC – Ablage auf Hostinger und Information

#### Ablauf

1. **Stammdokumente** liegen im Repo unter [docs/stammdokumente_oc/](../01_CORE_DNA/stammdokumente_oc/00_INDEX.md) (00_INDEX, 01_PROJEKT_CORE, 02_MARC_UND_TEAM, 03_OC_ROLLE_UND_GRENZEN).
2. **Freigabe durch den OMEGA_ATTRACTOR Council:** Der OMEGA_ATTRACTOR Council (Core Council) prüft die Stammdokumente. Wenn alle eine Stimme abgegeben haben und die Mehrheit dafür ist, gilt die Freigabe.
...
- Nur ausführen **nach** Freigabe durch den OMEGA_ATTRACTOR Council.
...
```
[CORE] Die Stammdokumente für dich liegen bereit. Sie befinden sich auf dem Server im OC-Workspace unter stammdokumente/ (00_INDEX.md, PROJEKT_CORE, MARC_UND_TEAM, OC_ROLLE_UND_GRENZEN). Dort steht, was wir hier machen, warum, wer Marc ist, wer das Team ist und wie deine Rolle und Grenzen definiert sind.
```

(Der Präfix [CORE] kennzeichnet Nachrichten vom vollen CORE/4D_RESONATOR (CORE); siehe WHATSAPP_E2E_HA_SETUP.md / DEV_AGENT_UND_SCHNITTSTELLEN – Abschnitt WhatsApp-Präfix.)
